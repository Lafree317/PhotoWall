...
var ControllerUnit = React.createClass({
  handleClick:function(e){
    // 如果点击的是当前正在选中的状态,则翻转图片,否则将对应的图片居中
    if (this.props.arrange.isCenter) {
      this.props.inverse();
    }else {
      this.props.center();
  ...
  render: function() {
    var controllerUniteClassName = "controller-unit";

    // 如果对应的是居中的图片,显示按钮的居中态
    if (this.props.arrange.isCenter) {
        controllerUniteClassName += " is-center";

        // 如果同时对应的是翻转图片,显示控制按钮的翻转态
        if (this.props.arrange.isInverse) {
          controllerUniteClassName += " is-inverse";
        }
    }

    return(
      <span className={controllerUniteClassName} onClick={this.handleClick}></span>
    )
  }
